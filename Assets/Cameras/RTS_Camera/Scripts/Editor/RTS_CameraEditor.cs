    ...
    public class RTS_CameraEditor : Editor
    {
        ...
        private void RotationTab()
        {
            ...
            if(camera.useKeyboardRotation)
            {
				...
				camera.tiltUpKey = (KeyCode)EditorGUILayout.EnumPopup("Tilt up: ", camera.tiltUpKey);
				camera.tiltDownKey = (KeyCode)EditorGUILayout.EnumPopup("Tilt down: ", camera.tiltDownKey);
        ...
        private void DistanceTab()
        {
            using (new HorizontalBlock())
            {
                GUILayout.Label("Auto distance: ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.autoDistance = EditorGUILayout.Toggle(camera.autoDistance);
            }
            if (camera.autoDistance)
            {
                camera.distanceDampening = EditorGUILayout.FloatField("Distance dampening: ", camera.distanceDampening);
            ...
            {
				camera.minDistance = EditorGUILayout.FloatField("Min Distance: ", camera.minDistance);
				camera.maxDistance = EditorGUILayout.FloatField("Max Distance: ", camera.maxDistance);
				camera.initialDistance = EditorGUILayout.FloatField("Initial Distance: ", camera.initialDistance);
            }  
        }
    }
}